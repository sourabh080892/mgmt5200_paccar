...
```{r}
ots_pdc <- ots %>% filter(channel == 'PDC')
```
...
```{r}
# rfill_rate = 1 - sum(rhit) / sum(rline)
metrics_summary <- demand_all %>%
  mutate(year = format(as.Date(order_month), "%Y"))
  group_by(year, marketing_channel) %>%
  summarise(rfill_rate = 1 - sum(rhit) / sum(rline),
        nfill_rate = 1 - sum(nhit) / sum(nline),
        inventory_cost = (sum(length * width * height / 1728) )* 5.5)
```
